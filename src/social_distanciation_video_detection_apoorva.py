...
#check the violators
#pairwise distance , create clusters
#calculate the number of downoids in that cluster


def check_present(clusters, element):
        if(len(clusters)==0):
                return False, -1
        for cl in clusters:
                if(element in cl):
                        return True, clusters.index(cl)
        return False, -1
	
def calc_dist(l1, l2):
        d = math.sqrt( (l1[0] - l2[0])**2 + (l1[1] - l2[1])**2 )
        if (d < int(distance_minimum)):
                return True
        return False

def violators(downoids):
        clusters = []
        k = 0
        for i in range(len(downoids)):
                x, y = check_present(clusters, downoids[i])
                if(x == True):
                        k = 1
                        clus = clusters[y]
                else:
                ...
                for j in range(i+1, len(downoids)):
                        if(calc_dist(downoids[i], downoids[j]) == True):
                                if(check_present(clusters, downoids[j]) != True):
                                        clus.append(downoids[j])

                if(len(clus) > 1 and k!=1):
                        clusters.append(clus)

        print('final violators clusters')
        print(clusters)
        num_vio = sum(len(row) for row in clusters)
        return num_vio, len(clusters), clusters	
...
while True:	
	...
	if not frame_exists:
		...
		if len(transformed_downoids) >= 2:
			...
			num_violators, num_clusters, clusters = violators(transformed_downoids)
			print("Nmber of violators", num_violators)
			print("Nmber of clusters", num_clusters)
