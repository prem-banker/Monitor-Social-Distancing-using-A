...
CLUSTER_COLORS = [ (0, 255, 255), (255, 255, 255), (255, 0, 255), (255,165,0), (0,0,128), (139,69,19)]
BIG_CIRCLE = 60
SMALL_CIRCLE = 3
...
def isneighbour(pair, distance_minimum):
...
while True:     
        ...
        if not frame_exists:
                ...
                if len(transformed_downoids) >= 2:
                        ...
                        for i in range(len(indexes)):
                                for index_pt1 in indexes[i]:
                                        cv2.circle(bird_view_img, (transformed_downoids[index_pt1][0], transformed_downoids[index_pt1][1]), BIG_CIRCLE, CLUSTER_COLORS[color], 2)
                                        cv2.circle(bird_view_img, (transformed_downoids[index_pt1][0], transformed_downoids[index_pt1][1]), SMALL_CIRCLE, CLUSTER_COLORS[color], -1)
                                        cv2.rectangle(frame,(array_boxes_detected[index_pt1][1],array_boxes_detected[index_pt1][0]),(array_boxes_detected[index_pt1][3],array_boxes_detected[index_pt1][2]),CLUSTER_COLORS[color],2)
                                color+=1
                                color%=(len(CLUSTER_COLORS))

                        frameppl = len(transformed_downoids)
                        frameclusters = []
                        frameviolators = 0
                        for el in answer:
                                if len(el) > 1:
                                        frameclusters.append(el)
                                        frameviolators+=len(el)
                        framenoc = len(frameclusters)

                        #print(' TOTAL NUMBER OF PEOPLE : ' , frameppl)
                        #print(' TOTAL NUMBER OF CLUSTERS : ' , framenoc)
                        #print(' TOTAL NUMBER OF VIOLATORS : ' , frameviolators)
                        #print('\n ======================= \n')
                        avgppl+=frameppl
                        avgclusters+=framenoc
                        avgviolators+=frameviolators
        ...
        if elapsedtime > 5:
                avgppl/=frame_no
                avgclusters/=frame_no
                avgviolators/=frame_no
                #print(avgppl, avgclusters, avgviolators)

                #avgppl = '{:.2f}'.format(avgppl)
                #avgclusters = '{:.2f}'.format(avgclusters)
                #avgviolators = '{:.2f}'.format(avgviolators)

                payload ='{:.1f}'.format(avgppl) + ' ' + '{:.1f}'.format(avgclusters) + ' ' + '{:.1f}'.format(avgviolators)
                print(payload)
                if avgviolators > 6 and avgclusters > 2:
                        print('BAD SITUATION')
                else:
                        print('NORMAL SITUATION')

                # SEND
                file = open("C:\wamp64\www/ajaxtext.txt", "r+")
                file.truncate(0)
                file.write(payload)
                file.close()
                frame_no = 0
